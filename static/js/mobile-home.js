/**
 * HuSNS Mobile Home Page
 * 移动端首页专用脚本 - 完全重写，解决数据加载问题
 * @version 4.0.0
 */
...
(function() {
    ...
    var HomeApp = {
        ...
        init: function(baseUrl) {
            ...
            console.log('[HomeApp] Initialized, currentTab:', this.config.currentTab);
        },

        // 从DOM同步tab状态
        syncTabFromDOM: function() {
            var activeTab = document.querySelector('.tab-item.active');
            if (activeTab && activeTab.dataset.tab) {
                this.config.currentTab = activeTab.dataset.tab;
                console.log('[HomeApp] Tab synced from DOM:', this.config.currentTab);
            }
        },
        ...
        handleRepost: function(btn) {
            ...
            console.log('[HomeApp] Repost clicked:', postId);
        ...
        handleLike: function(btn) {
            ...
            console.log('[HomeApp] Like clicked:', postId, 'action:', action);
            ...
            .catch(function(error) {
                console.error('[HomeApp] Like error:', error);
        ...
        switchTab: function(tabEl) {
            var newTab = tabEl.dataset.tab;
            var isSameTab = newTab === this.config.currentTab;
            console.log('[HomeApp] Switching tab from', this.config.currentTab, 'to', newTab, 'isSameTab:', isSameTab);

            // 更新状态
            this.config.currentTab = newTab;
        ...
        // 加载微博列表
        loadPosts: function(isLoadMore) {
            var self = this;
            
            if (this.config.loading) {
                console.log('[HomeApp] Already loading, skip');
                return;
            }

            this.config.loading = true;
            console.log('[HomeApp] Loading posts, tab:', this.config.currentTab, 'page:', this.config.currentPage);
            ...
            // 构建API URL
            var url = this.config.baseUrl + '/?r=mobileApi/posts' + 
                      '&page=' + this.config.currentPage + 
                      '&tab=' + encodeURIComponent(this.config.currentTab);

            console.log('[HomeApp] API URL:', url);
            ...
            .then(function(response) {
                console.log('[HomeApp] Response status:', response.status);
                return response.json();
            })
            .then(function(data) {
                console.log('[HomeApp] Response data:', data);
            ...
            .catch(function(error) {
                console.error('[HomeApp] Load error:', error);
                self.config.loading = false;
                self.hideLoading();
        ...
        renderPosts: function(posts, isAppend) {
            var container = document.getElementById('postList');
            if (!container) {
                console.error('[HomeApp] Container not found');
                return;
            }
